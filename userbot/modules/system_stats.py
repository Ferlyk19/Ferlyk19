...
async def bot_ver(event):
    ...
    if which("git") is not None:
        ...
        await event.edit(
            "**⚜-**⚡Ferly-Project⚡ Versi:** \n "
            f"heads/Ferly-Userbot-0-x634i7u1"
            "\n**⚜-**Revisi:**\n "
            f"{revout}"
        )
    else:
...
async def amireallyalive(alive):
    ...
    await get_readable_time((time.time() - StartTime))
    output = (
        f"**ㅤㅤㅤㅤ ⚡𝔽𝕖𝕣𝕝𝕪 - ℙ𝕣𝕠𝕛𝕖𝕔𝕥⚡**\n"
        f"╔══════════╬════╬══════════╗ \n"
        f"╟⟩⟩ 🤴 • `ᴏᴡɴᴇʀ    :`[𝔽𝕖𝕣𝕝𝕪](t.me/FK190202)             ㅤ ║\n"
        ...
        f"╚══════════╬════╬══════════╝ \n"
        f"🔰 : [𝗥𝗘𝗣𝗢-𝙂𝙚𝙚𝙯](https://github.com/Ferlyk19/ferlyk19-UserBot)\n👥 : [ＧΞ ΞＺ-𝗧𝗘𝗔𝗠](t.me/alivesholarssociety)\n👤 : [𝗢𝗪𝗡𝗘𝗥](t.me/FK190202)\n"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
...
async def redis(alive):
    ...
    output = (
        ...
        f"╭═───────╼⌘╾───────═ \n"
        f"┃[𝗥𝗲𝗽𝗼](https://github.com/Ferlyk19)  |  [𝗦𝘂𝗽𝗽𝗼𝗿𝘁](t.me/alivesholarssociety)  |  "
        f"[𝗜𝗻𝘀𝘁𝗮𝗴𝗿𝗮𝗺](https://Instagram.com/er.19__)\n"
        f"╰═──────╼═⌘═╾───────═")
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
